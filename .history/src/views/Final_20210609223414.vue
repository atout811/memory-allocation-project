...
export default {
  ...
  data() {
    return {
      ...
      holes: this.$store.state.input.Holes.map((item) => [
        item.starting,
        item.size,
      ]),
      test: this.$store.state.input.Processes.map((item) => [
        null,
        item.segments.forEach((el) => {
          Object()[el.name] = [Number(el.size)];
        }),
        item.name,
      ]),
      processes: this.test.map((item) => [null, obj, item[2]]),
    };
  },
  mounted: function () {
