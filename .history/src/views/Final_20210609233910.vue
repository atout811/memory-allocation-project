...
export default {
  ...
  mounted: function () {
    ...
    const scale = 2;
    let data = { ...this.data };
    let holes = data.Holes.map((item) => [item.starting, item.size]);
    // let holes = [
    //   [140, 100],
    //   [40, 80],
    //   [260, 90],
    ...
    let totalMemorySize = data.memSize;
    function orderHoles() {
    ...
    // ]);
    let test = [];
    for (let index = 0; index < data.Processes; index++) {
      item = data.Processes[index].segments;
      temp = {};
      for (let j = 0; j < item; j++) {
        temp[item[j].name] = item[j].size;
      }
    ...
    // ]; //start,size,name
    console.log("prooooooooooocess: ", test);
    let OldProcess = [];
